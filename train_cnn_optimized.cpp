...
TODO
- only one bias per frame on the convolution layer
- use 8 frames on conv and max layer
- no weights between conv and max layer
- no bias on the max layer
...
float s_m[6][12][12] = {0};   //max layer without weight
...
void descend(float eta)
{
    ...
    for (int signode = 0; signode < 45; signode++)
    {
        float gradient1 = sum_g0;
        float g1End = 0;
        for (int node = 0; node < 10; node++)
            g1End += v[signode][node] * deltaSigmoid(y_sig[signode]);
        gradient1 = gradient1 * g1End;
    ...
    for (int maxnode = 0; maxnode < 6; maxnode++)
    {
        for (int mx = 0; mx < 12; mx++)
        {
            for (int my = 0; my < 12; my++)
            {
                ...
                for (int signode = 0; signode < 45; signode++){
                    float gradient2 = sum_g1 * u[maxnode][mx][my][signode];
                    sum_g2 += gradient2;
                }

                float relevant_y_c = s_m[maxnode][mx][my];
    ...
    float sum_g3 = 0;
    for (int convnode = 0; convnode < 6; convnode++)
    {
        for (int cx = 0; cx < 24; cx++)
        {
            for (int cy = 0; cy < 24; cy++)
            {
                if (s_m[convnode][cx/2][cy/2] == y_c[convnode][cx][cy])
                {
                    float gradient3 = sum_g3 * y_c[convnode][cx][cy]; // verify this
                    b_c[convnode][cx][cy] = b_c[convnode][cx][cy] - gradient3;
                    sum_g3 += gradient3;
                }
            }
        }
